...
const GetCentralGeoCoordinate = (geoCoordinates: Place[]) => {
  ...
  for (var i = 0; i < geoCoordinates.length; i++) {
    const geoCoordinate = geoCoordinates[i].geometry?.location;
    if (geoCoordinate) {
...
var svgNumberMarkup = () => `<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#000000"><path d="M0 0h24v24H0z" fill="none"/><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>`
...
interface HereMapProps {
  current?: LatLng;
  items: PropertyAttributes[] | CustomerAttributes[] | RunnerAttributes[];
  waypoints: Waypoint[];
  mapHeight: number;
}

const HereMap = ({ mapHeight, current, items, waypoints, ...rest }: HereMapProps) => {
  ...
  const places: Place[] = useMemo(() => {
    return items
      ...
      // })
      .filter((item: Property | Customer | Runner) => {
        if (item.place) return item;
      })
      .map((item: Property | Customer | Runner) => {
        if (item.place && item.place.geometry) return item.place;
  ...
  useLayoutEffect(() => {
    ...
    const initialPos = places[0] && places[0].geometry?.location || null;
    ...
    if (places.length > 1) {
      ...
      places.map((place: Place, index: number) => {
        ...
        const marker = new H.map.Marker(place.geometry?.location, { icon });
  ...
  return <Box className="map" width={'100%'} height={'500px'} ref={mapRef} {...rest} />;
};
